module Model.Interval exposing (Interval, compare, full, length, oneYear, open, withDurationMonths, withDurationYears,view)
...
import Model.Date exposing (datetoString)
...
compare (Interval intA) (Interval intB) =
    let
        startComparison = Date.compare intA.start intB.start
        endComparison =
    ...
    in
    chainCompare startComparison endComparison
...
view interval =
    let 
        ...
        startDateHtml =
            p [ class "interval-start" ] [ text <| Date.datetoString i.start ]
        ...
        endDateHtml =
            p [ class "interval-end" ]
                [ text
                    <| case i.end of
                        Just endDate -> Date.datetoString endDate
                        Nothing -> "Present"
                ]
